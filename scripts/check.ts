...
async function listChangedFiles() {
  ...
  const trackedFiles = hasHead
    ? (
        await $`git diff --name-only --diff-filter=ACMR HEAD`
          .cwd(ROOT)
          .text()
      )
        .trim()
...
async function runCpd() {
  ...
  await $`jscpd --reporters json --output ${outDir}`.cwd(ROOT).quiet().nothrow()
...
async function runTests() {
  const hasTests =
    (
      await $`find tests -name '*.test.ts' -o -name '*.test.tsx' 2>/dev/null`
        .cwd(ROOT)
        .nothrow()
        .text()
    ).trim().length > 0
